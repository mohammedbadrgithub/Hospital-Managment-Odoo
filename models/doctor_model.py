...
class DoctorModel(models.Model):
    ...
    def computed_tourist_age(self):
        for record in self :
            if record.birthdate:
                ...
                if record.age <  23 :
                    raise ValidationError('doctor can not be little like that ! ')
            else :
                record.age = 0
    
    specilize =  fields.Many2one('hms.specilize',required=True)
    ...
    # validate age doctors can not be less than special age
    # @api.constrains('age')
